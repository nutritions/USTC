...
class StartPage:
    def __init__(self, parent_window):
        ...
        self.window.geometry('360x640+%d+%d' % ((self.window.winfo_screenwidth() // 2 - 150), (self.window.winfo_screenheight() // 2 - 300)))

        label = Label(self.window, text="学生信息管理系统", font=("Verdana", 20))
        label.pack(pady=60)  # pady=100 界面的长度

        # try:
        #     self.logo = PhotoImage(file="ustclogo.png")  # 确保图片文件名和路径正确
        ...
        Button(self.window, text="管理员登陆", font=tkFont.Font(size=16), command=lambda: AdminPage(self.window), width=20,
               height=2,
               fg='white', bg='blue', activebackground='black', activeforeground='white').pack(pady=20)
        
        Button(self.window, text="学生登陆", font=tkFont.Font(size=16), command=lambda: StudentPage(self.window), width=20,
               height=2, fg='white', bg='blue', activebackground='black', activeforeground='white').pack(pady=20)
        Button(self.window, text="账号系统", font=tkFont.Font(size=16), command=lambda: AboutPage(self.window),
               width=20,
               height=2,
               fg='white', bg='blue', activebackground='black', activeforeground='white').pack(pady=20)
        Button(self.window, text='退出系统', height=2, font=tkFont.Font(size=16), width=20, command=self.window.destroy,
               fg='white', bg='blue', activebackground='black', activeforeground='white').pack(pady=20)
...
class AdminPage:
    def __init__(self, parent_window):
        ...
        label = tk.Label(self.window, text='管理员登陆', bg='white', font=('隶书', 20, 'bold'), width=30, height=2)
        label.pack(pady=20)
    ...
    def login(self):
        ...
        # 数据库操作 查询管理员表
        db = pymysql.connect(host='localhost', port=3306, db='test', user='root', password='lzc20030210') # 打开数据库连接
...
class AdminManage:
    def __init__(self, parent_window):
        ...
        # 定义下方中心列表区域
        self.columns = ("学号", "姓名", "性别", "年龄","专业","成绩")
        ...
        # 表格的标题
        self.tree.column("学号", width=80, anchor='center')  # 表示列,不显示
        ...
        self.tree.column("专业", width=80, anchor='center')
        ...
        self.grade = []
        # 打开数据库连接
        db = pymysql.connect(host='localhost', port=3306, db='test', user='root', password='lzc20030210')
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        sql = "SELECT s.id, s.name, s.gender, s.age, m.MajorName FROM student s JOIN Major m ON s.MajorID = m.MajorID"  # SQL 查询语句
        try:
            # 执行SQL语句
            cursor.execute(sql)
            # 获取所有记录列表
            ...
            for row in results:
                ...
                self.major.append(row[4])
                self.grade.append(str(row[5]))
        ...
        print("test***********************")
        for i in range(min(len(self.id), len(self.name), len(self.gender), len(self.age),len(self.major),len(self.grade))):  # 写入数据
            self.tree.insert('', i, values=(self.id[i], self.name[i], self.gender[i], self.age[i],self.major[i],self.grade[i]))
        ...
        self.right_top_gender_entry = Entry(self.frame_left_top, textvariable=self.var_major,
                                            font=('Verdana', 15))
        ...
        self.right_top_gender_entry = Entry(self.frame_left_top, textvariable=self.var_grade,
                                            font=('Verdana', 15))
    ...
    def click(self, event):
        ...
        self.var_major.set(self.row_info[4])
        self.var_grade.set(self.row_info[5])
    ...
    def new_row(self):
        ...
        if str(self.var_id.get()) in self.id:
            ...
            if self.var_id.get() != '' and self.var_name.get() != '' and self.var_gender.get() != '' and self.var_age.get() != '' and self.var_major.get() != '' and self.var_grade.get() != '':
                # 打开数据库连接
                db = pymysql.connect(host='localhost', port=3306, db='test', user='root', password='lzc20030210')
                cursor = db.cursor()  # 使用cursor()方法获取操作游标
                sql = "INSERT INTO student(id, name, gender, age) \
				       VALUES ('%s', '%s', '%s', '%s')" % \
                      (self.var_id.get(), self.var_name.get(), self.var_gender.get(), self.var_age.get(),self.var_major.get(),self.var_grade.get())  # SQL 插入语句
                sqls = "INSERT INTO `student_login` VALUES ('%s', '111111')" % (self.var_id.get())
                try:
                ...
                self.major.append(self.var_major.get())
    ...
    def updata_row(self):
        ...
        if res == True:
            if self.var_id.get() == self.row_info[0]:  # 如果所填学号 与 所选学号一致
                ...
                db = pymysql.connect(host='localhost', port=3306, db='test', user='root', password='lzc20030210')
                cursor = db.cursor()  # 使用cursor()方法获取操作游标
                sql_update = "UPDATE student SET name = '%s', gender = '%s', age = '%s' \
				 WHERE id = '%s'" % (
                    self.var_name.get(), self.var_gender.get(), self.var_age.get(), self.var_id.get(),self.var_major.get(),self.var_grade.get())  # SQL 插入语句
                try:
                ...
                self.major[id_index] = self.var_major.get()
                self.grade[id_index] = self.var_grade.get()

                self.tree.item(self.tree.selection()[0], values=(
                    self.var_id.get(), self.var_name.get(), self.var_gender.get(),
                    self.var_age.get(),self.var_major.get(),self.var_grade.get()))  # 修改对于行信息
            else:
    ...
    def del_row(self):
        ...
        if res == True:
            ...
            db = pymysql.connect(host='localhost', port=3306, db='test', user='root', password='lzc20030210')
            cursor = db.cursor()  # 使用cursor()方法获取操作游标
            sql_delete = "DELETE FROM student WHERE id = '%s'" % (self.row_info[0])  # SQL 插入语句
            sql_deletes = "DELETE  FROM student_login WHERE stu_id = '%s'" % (self.row_info[0])  # SQL 插入语句
            try:
...
class StudentView:
    def __init__(self, parent_window, student_id):
        ...
        label = tk.Label(self.window, text='学生信息', bg='silver', font=('Verdana', 20), width=30, height=2)
        label.pack(pady=20)
        ...
        db = pymysql.connect(host='localhost', port=3306, db='test', user='root', password='lzc20030210')
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        sql = "SELECT s.id, s.name, s.gender, s.age, m.MajorName,s.grade FROM student s JOIN Major m ON s.MajorID = m.MajorID where s.id= '%s'" % (student_id)  
        ...
        Label(self.window, text=self.major, font=('Verdana', 18)).pack(pady=5)
        Label(self.window, text=self.grade, font=('Verdana', 18)).pack(pady=5)
...
class Changekey:
    ...
    def login(self):
        ...
        stu_pass = None
        # 数据库操作 查询管理员表
        db = pymysql.connect(host='localhost', port=3306, db='test', user='root', password='lzc20030210')  # 打开数据库连接   已经修正删除多余空格
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        sql = "SELECT * FROM student_login WHERE stu_id = '%s'" % (self.admin_id.get())  # SQL 查询语句
        try:
            # 执行SQL语句
            cursor.execute(sql)
            # 获取所有记录列表
    ...
    def chage(self):
        ...
        db = pymysql.connect(host='localhost', port=3306, db='test', user='root', password='lzc20030210')
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        sql = "UPDATE student_login SET stu_pass = '%s' WHERE stu_id = '%s'" % (
...
class CreateAdminPage:
    ...
    def find(self):
        ...
        db = pymysql.connect(host='localhost', port=3306, db='test', user='root', password='lzc20030210') # 打开数据库连接
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        sql = "SELECT admin_id FROM admin_login WHERE admin_id = '%s'" % \
    ...
    def change(self):
        ...
        db = pymysql.connect(host='localhost', port=3306, db='test',user='root', password='lzc20030210') # 打开数据库连接
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        sql = "INSERT INTO admin_login(admin_id, admin_pass) VALUES ('%s', '%s')" % \
...
class AdminChange:
    ...
    def chage(self):
        ...
        db = pymysql.connect(host='localhost', port=3306, db='test', user='root', password='lzc20030210')  # 打开数据库连接
        cursor = db.cursor()  # 使用cursor()方法获取操作游标
        sql = "UPDATE admin_login SET admin_pass = '%s' WHERE admin_id = '%s'" % \
