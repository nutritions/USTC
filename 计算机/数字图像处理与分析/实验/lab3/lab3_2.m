...
poisson_output = filter(random,30);
...
subplot(3,3,7); imshow(pepper_output); title('椒盐超限邻域'); 
subplot(3,3,8); imshow(gaussian_output); title('高斯超限邻域'); 
subplot(3,3,9); imshow(poisson_output); title('随机超限邻域'); 
...
function [output] = filter(input,T)
    ...
    for i = 2 : (r - 1)
        for j = 2 : (l - 1)
            mean_value = mean(mean(input(i - 1 : i + 1, j - 1 : j + 1)));
